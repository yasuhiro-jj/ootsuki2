...
👉 まずは **生シラス** と **桜えびかき揚げ** のセットがおすすめ！"""
            ...
            logger.info("[Sashimi] option_click: 海鮮刺身（固定文・いくら/真鯛/うに削除）")
            response_text = """🍽️ **海鮮刺身**
🚢 店長が沼津港から直接仕入れ！トロール船から引き揚げられる新鮮な魚をお刺身でご提供しています。

• **刺身盛り合わせ** ¥1,600
鮮度・品数・バランス良し。シェアにも最適。
沼津港の旬の魚たちを船盛でご用意いたしております。ぜひ皆さんで召し上がってください。

• **まぐろの中おち刺** ¥490
旨み濃厚、まぐろ中おちを贅沢に。
骨周りの旨みが詰まった中おちを新鮮なまま刺身で。ねっとりした口当たりと濃い旨みが魅力です。わさび醤油や海苔巻き風にしても相性抜群。

• **富士山サーモン刺** ¥680
とろける旨脂と香り。地元素材の自信作。
富士宮で育てられた地元独自のサーモンです。キレのある味わいをぜひ召し上がってください。

👉 まずは **刺身盛り合わせ** でシェアがおすすめ！"""
            state["response"] = self._add_order_instruction(response_text)
            # 従来の海鮮刺身ノード系と同系統の横断導線
            state["options"] = [
                "天ぷらメニュー確認",
                "エビフライ",
                "アジフライ",
