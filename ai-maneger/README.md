...
[https://ootsuki-ai-maneger.vercel.app/login?tenant=demo](https://ootsuki-ai-maneger.vercel.app/login?tenant=demo](https://ootsuki-ai-maneger.vercel.app/login?tenant=demo)) 
...
[http://localhost:3002/dashboard?tenant=ootsuki](http://localhost:3002/dashboard?tenant=ootsuki)

[http://localhost:3002/dashboard?tenant=demo](http://localhost:3002/dashboard?tenant=demo)
...
[https://ai-maneger.vercel.app/dashboard](https://ai-maneger.vercel.app/dashboard)

[https://ai-maneger.vercel.app/dashboard?tenant=ootsuki](https://ai-maneger.vercel.app/dashboard?tenant=ootsuki)
...
[http://localhost:3002/login?tenant=demo](http://localhost:3002/login?tenant=demo](http://localhost:3002/login?tenant=demo))
...
AUTH_[CREDENTIALS.md](http://CREDENTIALS.md) には ootsuki 専用の別ユーザー名は書かれておらず、本番運用側で使うのは 管理者 owner です（「管理者（ローカル開発用）」として記載）。
...
Supabase を使う場合の取得手順は [docs/tenant-config-supabase.md](docs/[tenant-config-supabase.md](http://tenant-config-supabase.md)) を参照してください。
...
| **設定を DB に寄せる（推奨）** | 上記「段階1」を有効にし、テナント固有のトークン・各 DB ID を `**tenant_configs`（PostgreSQL）** に保存する。ローカル／本番の `.env` には共通の接続情報`TENANT_CONFIG_DB_URLAPP_CONFIG_ENCRYPTION_KEY` など）だけを置ける。詳細は [docs/tenant-config-supabase.md](docs/[tenant-config-supabase.md](http://tenant-config-supabase.md))。 |
...
1. [Supabase]([https://supabase.com/dashboard](https://supabase.com/dashboard)) でプロジェクト作成。
2. **Project Settings → Database → Connection string（URI）** を取得し、パスワードを埋める。
3. `.env.local`（本番は Vercel の Environment Variables）に `TENANT_CONFIG_STORE_ENABLED=trueTENANT_CONFIG_DB_URLAPP_CONFIG_ENCRYPTION_KEY`（32文字以上のランダム。**Notion のトークンとは別**）を設定。
4. ルートで `npm run migrate:tenant-config` → `npm run seed:tenant-config` → `npm run check:notion-env -- --tenant=demo` などで確認。

詳細・トラブルシュート（IPv6／Pooler）は [docs/tenant-config-supabase.md](docs/[tenant-config-supabase.md](http://tenant-config-supabase.md)) に一元化済み。
...
| 解決・環境      | `lib/tenant-config/service.ts`                                                            | `normalizeTenantKeygetEnvTenantNotionConfigbuild`*（新テナント用の `NOTION_`* プレフィックス） |

| Middleware | `middleware.ts`                                                                           | 同上の許可キー／ホスト名ルール                                                                      |

| API の検証    | `app/api/admin/tenant-config/route.tstenant-memberships` など `validateTenantKey` が二択の箇所 | 3値に拡張                                                                                |

| 管理 UI      | `app/admin/tenant-access/page.tsxcomponents/admin/tenant-membership-manager.tsx`       | プルダウン／フィルタ                                                                           |

| Scripts    | `scripts/seed-tenant-config.tsscripts/check-notion-env.ts`                             | 対象 tenant のリスト                                                                       |
...
1. [Vercel]([https://vercel.com/](https://vercel.com/)) にログインし、**Add New… → Project** でリポジトリをインポートする。
...
4. **Environment Variables** に、ローカルの `.env.local` と同じキーを登録する（本番・プレビューどちらに入れるかは用途に合わせて選択）。
  - 必須（Notion）: `NOTION_API_KEYNOTION_PROJECT_DB_IDNOTION_OOTSUKI_PROJECT_PAGE_IDNOTION_OOTSUKI_DAILY_SALES_DB_IDNOTION_OOTSUKI_KPI_DB_IDNOTION_OOTSUKI_MEMO_DB_IDNOTION_OOTSUKI_LINE_REPORT_PAGE_IDNOTION_OOTSUKI_PRODUCT_COST_DB_IDNOTION_OOTSUKI_WEEKLY_ACTIONS_DB_ID`（任意で `NOTION_OOTSUKI_INSTRUCTIONS_PAGE_ID`＝ダッシュボードの運用指示書）
  - 必須（個別認証）: `APP_AUTH_SESSION_SECRETAPP_AUTH_USERS_JSON`
  - 推奨: `NEXT_PUBLIC_APP_NAME`
  - 任意: `NOTION_API_TOKENNOTION_API_KEY` の代わり／併用のフォールバック）
...
プロジェクトルートで [Vercel CLI]([https://vercel.com/docs/cli](https://vercel.com/docs/cli)) を入れたうえで:
...
[docs/notion-demo/cursor-report-notion-db-audit-2026-04-29.md](docs/notion-demo/[cursor-report-notion-db-audit-2026-04-29.md](http://cursor-report-notion-db-audit-2026-04-29.md))
...
### なぜ `/login?tenant=ootsuki` （および `tenant=demo`）でログインできるか

`ootsuki` も `**demo` も同じタイプ**です。クエリだけでテナントが切り替わり、そのテナント用の認証・Notion を使います。（実装は `middleware.ts` の公開ページ判定と、`?tenant=` 時の Cookie 保存）

1. `**/login` は認証 Cookie がなくても表示できる**公開ページ。**未ログインでも**ログインフォームに直行できる。
2. `**?tenant=ootsuki` / `?tenant=demo` を付けると `tenant_key` Cookie がそれぞれ保存*され、ログイン後は `**ootsuki` … `NOTION_OOTSUKI_` 等、`demo` … `NOTION_DEMO_`* 等**（`tenant_configs` があればそちら）を参照する。
3. **そのホストのデプロイ**に、`APP_AUTH_SESSION_SECRET` と `**APP_AUTH_USERS_JSON` が正しい JSON 配列で**設定されていれば、`demo-viewer` や `owner` など定義済みユーザーでログインできる。
