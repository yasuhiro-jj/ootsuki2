...
export function aggregateMonth(entries: KpiSnapshotEntry[], referenceDate: Date): MonthlyAggregate {
  ...
  const monthEntries = entries.filter(
    (entry) => entry.date && entry.date >= monthStart && entry.date <= monthEnd,
  );
...
export function buildProfitActionAlerts(summary: ProfitActionSummary): ProfitActionAlert[] {
  ...
  if (typeof grossMarginRateDelta === "number" && grossMarginRateDelta <= -1.5) {
    alerts.push({
      title: "粗利率が悪化",
      status: "urgent",
      reason: `粗利率が前月比 ${formatPercentDelta(grossMarginRateDelta)} です。値引き/原価上振れの確認が必要です。`,
      actions: [
  ...
  if (typeof averageSpendDelta === "number" && averageSpendDelta <= -4) {
    alerts.push({
      title: "客単価が低下",
      status: "urgent",
      reason: `客単価が前月比 ${formatPercentDelta(averageSpendDelta)} です。追加注文導線の弱化が疑われます。`,
      actions: [
  ...
  if (typeof customersDelta === "number" && customersDelta <= -6) {
    alerts.push({
      title: "客数が減少",
      status: "watch",
      reason: `客数が前月比 ${formatPercentDelta(customersDelta)} です。来店動機の再提示が必要です。`,
      actions: [
