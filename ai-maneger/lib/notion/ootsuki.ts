...
const BUDGET_KEYS = ["当年予算", "予算", "Budget"];
const ACHIEVEMENT_RATE_KEYS = ["当年達成率", "達成率", "Achievement Rate"];
...
function mapKpiEntry(page: NotionPage): KpiSnapshotEntry {
  ...
  return {
    ...
    budget: getPropertyNumber(page.properties, BUDGET_KEYS) ?? undefined,
    achievementRate: getPropertyNumber(page.properties, ACHIEVEMENT_RATE_KEYS) ?? undefined,
    previousSales: getPropertyNumber(page.properties, PREVIOUS_SALES_KEYS) ?? undefined,
    previousCustomers: getPropertyNumber(page.properties, PREVIOUS_CUSTOMERS_KEYS) ?? undefined,
    previousAverageSpend: getPropertyNumber(page.properties, PREVIOUS_AVERAGE_SPEND_KEYS) ?? undefined,
...
export async function saveDailyInput(payload: DailyInputPayload) {
  ...
  setMappedProperty(properties, schemaProperties, BUDGET_KEYS, { number: payload.budget ?? null });
  setMappedProperty(properties, schemaProperties, ACHIEVEMENT_RATE_KEYS, { number: payload.achievementRate ?? null });
  setMappedProperty(properties, schemaProperties, PREVIOUS_SALES_KEYS, { number: payload.previousSales ?? null });
  setMappedProperty(properties, schemaProperties, PREVIOUS_CUSTOMERS_KEYS, { number: payload.previousCustomers ?? null });
  setMappedProperty(properties, schemaProperties, PREVIOUS_AVERAGE_SPEND_KEYS, { number: payload.previousAverageSpend ?? null });
