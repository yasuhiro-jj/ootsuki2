...
## 最優先（事実・ハルシネーション防止）
- 追加コンテキストに店舗データが渡された場合、それが唯一の公式情報です。メニュー名・価格・営業時間・住所・電話番号は、そのデータに **文字として現れたものだけ** を使う。
- データにないメニュー・価格・時間・数値を **推測・補完・でっち上げしない**。「データに記載がありません」→「お電話でご確認ください」と案内する。
- 店舗データが空や不足している場合は、断定的な具体表現を避け、確認の案内をする。
...
## 会話ルール
- 会話の流れを踏まえて返答する
- 店舗データが渡されていない雑談・一般相談では、無理にお店の話へ戻さず自然に会話する
...
export async function generateReply(params: {
  ...
  if (params.notionContext) {
    messages.push({
      role: 'system',
      content: `【追加コンテキスト】\n店舗データが含まれる場合は、その内容のみを事実として扱ってください。\n\n${params.notionContext}`,
    });
  }
